...
export const SettingsModal: React.FC<SettingsModalProps> = ({
  ...
  const tabs: { id: TabId; label: string; icon: React.FC<any> }[] = [
    ...
    { id: 'connection', label: 'Server & Cloud', icon: Server },
              ...
              <div className="space-y-6 animate-in slide-in-from-right-4 fade-in duration-300">
                <div className="space-y-2">
                  <label className="text-xs font-semibold uppercase tracking-wider text-zinc-300 flex items-center gap-2">
                    ...
                    <span>Gemini & Backend Server Connection</span>
                  </label>
                  <p className="text-[12px] text-zinc-400 leading-relaxed">
                    Configure the cloud endpoint or local server that powers Compendium AI queries and Steam sync.
                  </p>
                </div>
                ...
                <div className="p-3.5 rounded-xl bg-black/60 border border-white/15 flex items-center justify-between">
                  <div className="space-y-0.5">
                    <div className="text-[11px] uppercase tracking-wider font-semibold text-zinc-400">Current Target Endpoint</div>
                    <div className="text-xs font-mono text-white font-semibold break-all">
                      {getApiBaseUrl() || (typeof window !== 'undefined' ? window.location.origin : 'http://localhost:3000')}
                    </div>
                  </div>
                  <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-white/5 border border-white/10 text-[11px] text-zinc-300">
                    <Wifi className="w-3 h-3 text-[var(--accent-color)]" />
                    <span>{customBackendUrl ? 'Custom Endpoint' : 'Default Preset'}</span>
                  </div>
                </div>

                {/* Endpoint Input & Actions */}
                <div className="space-y-3">
                  ...
                  <div className="flex flex-col sm:flex-row gap-2">
                    <input
                      ...
                      placeholder="https://your-published-app.run.app"
                      className="flex-1 px-3.5 py-2.5 rounded-xl bg-black/50 border border-white/15 text-xs text-white placeholder:text-zinc-600 focus:outline-none focus:border-[var(--accent-color)] font-mono"
                    />
                    <button
                      type="button"
                      onClick={() => {
                      ...
                      className="px-4 py-2.5 rounded-xl bg-[var(--accent-color)] text-black text-xs font-bold hover:opacity-90 transition-all cursor-pointer whitespace-nowrap"
                    >
                      Save & Apply
                    </button>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2 pt-1">
                    <button
                      ...
                      onClick={() => {
                        playBlipSound(soundEnabled);
                        setCustomBackendUrl(DEFAULT_PREVIEW_URL);
                        setApiBaseUrl(DEFAULT_PREVIEW_URL);
                        setTestResult(null);
                      }}
                      className="px-2.5 py-1.5 rounded-lg bg-white/5 hover:bg-white/10 border border-white/10 text-[11px] text-zinc-300 transition-colors cursor-pointer flex items-center gap-1.5"
                    >
                      <Globe className="w-3 h-3 text-purple-400" />
                      Published Cloud (Default)
                    </button>
                    <button
                      type="button"
                      onClick={() => {
                        playBlipSound(soundEnabled);
                        setCustomBackendUrl('http://localhost:3000');
                    ...
                    {customBackendUrl && (
                      <button
                        ...
                        className="px-2.5 py-1.5 rounded-lg bg-red-500/10 hover:bg-red-500/20 border border-red-500/20 text-[11px] text-red-400 transition-colors cursor-pointer"
                      >
                        Reset to Default
                      </button>
                    )}
                ...
                <div className="space-y-3 pt-2 border-t border-white/[0.08]">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-semibold text-zinc-200">Server Health Diagnostic</span>
                    <button
                      type="button"
                      ...
                      onClick={async () => {
                        ...
                        setTestResult(null);
                        const res = await testBackendHealth(customBackendUrl);
                        setTestResult(res);
                        setTestingHealth(false);
                      }}
                      className="px-3 py-1.5 rounded-lg bg-white/10 hover:bg-white/15 text-white text-xs font-semibold transition-all cursor-pointer flex items-center gap-1.5 disabled:opacity-50"
                    >
                      <RefreshCw className={`w-3.5 h-3.5 ${testingHealth ? 'animate-spin text-[var(--accent-color)]' : ''}`} />
                      {testingHealth ? 'Testing (max 6s)...' : 'Test Connection'}
                    </button>
                  </div>
                    ...
                    <div className={`p-3 rounded-xl border flex items-start gap-2.5 ${
                      ...
                      <div className="text-xs space-y-1">
                        <div className="font-semibold">
                          {testResult.ok ? 'Connection Successful (HTTP 200)' : 'Connection Test Failed'}
                        </div>
                        <div className="text-[11px] opacity-90 leading-relaxed">
                          {testResult.ok 
                            ? `Connected to Gemini server. Gemini Key is ${testResult.hasGeminiKey ? 'Active' : 'Unconfigured'}.` 
                            : (testResult.error || 'Server returned an error status.')}
                        </div>
                      </div>
                    </div>
                  )}
                ...
                <div className="p-3.5 rounded-xl bg-purple-500/10 border border-purple-500/20 space-y-2">
                  <div className="text-xs font-bold text-purple-200 flex items-center gap-1.5">
                    ...
                    How to get your live Cloud URL:
                  </div>
                  <ul className="text-[11px] text-purple-300/90 space-y-1.5 list-disc pl-4 leading-relaxed">
                    <li>
                      <strong>If you used Publish:</strong> When the Publish wizard finished deploying, it displayed a URL (e.g. <code>https://&lt;app&gt;.run.app</code>). Paste that URL above and click <em>Save & Apply</em>.
                    </li>
                    <li>
                      <strong>If you want the Shared Preview:</strong> Click the <strong>Share</strong> button in Google AI Studio (top header bar) to generate the shared preview, then select <em>Shared Preview (ais-pre)</em> above.
                    </li>
                    <li>
                      <strong>Running on PC locally:</strong> Run <code>npm run dev</code> in your terminal and select <em>Localhost:3000</em>.
                    </li>
                  </ul>
                </div>
              </div>
