...
// 1. Remove the entire buggy popup logic from the last patch
...
// 2. We need to add the /desktop-login HTML endpoint back to the authServer.
// We find where the authServer is defined.
...
const newEndpoint = `
  if (req.url === '/desktop-login') {
    ...
    res.end([
      ...
      "      authDomain: \\"gen-lang-client-0366642934.firebaseapp.com\\" // Use the proper authDomain!",
      "    };",
      "    ",
...
if (authCallbackIndex !== -1) {
  content = content.slice(0, authCallbackIndex) + newEndpoint + content.slice(authCallbackIndex);
}

// 3. Make sure the auth server binds explicitly to localhost
content = content.replace("authServer.listen(0, '127.0.0.1', () => {", "authServer.listen(0, 'localhost', () => {");
// In case it was already localhost from the very first patch that got overwritten:
if (!content.includes("authServer.listen(0, 'localhost'")) {
  content = content.replace("authServer.listen(0, () => {", "authServer.listen(0, 'localhost', () => {");
}


// 4. Add the IPC handler that opens the user's SYSTEM browser
const newIpcHandler = `ipcMain.on('start-desktop-login', () => {
